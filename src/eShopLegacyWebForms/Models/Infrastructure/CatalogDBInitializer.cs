...
namespace eShopLegacyWebForms.Models.Infrastructure
{
    public class CatalogDBInitializer : CreateDatabaseIfNotExists<CatalogDBContext>
    {
        ...
        public CatalogDBInitializer(CatalogItemHiLoGenerator indexGenerator)
        {
            ...
            useCustomizationData = bool.Parse(ConfigurationManager.Configuration.GetSection("appSettings")["UseCustomizationData"]);
        }
        ...
        {
            var preconfiguredTypes = useCustomizationData ? GetCatalogTypesFromFile() : PreconfiguredData.GetPreconfiguredCatalogTypes();
        ...
        {
            var preconfiguredBrands = useCustomizationData ? GetCatalogBrandsFromFile() : PreconfiguredData.GetPreconfiguredCatalogBrands();
        ...
        {
            var preconfiguredItems = useCustomizationData ? GetCatalogItemsFromFile(context) : PreconfiguredData.GetPreconfiguredCatalogItems();
        ...
        private IEnumerable<CatalogType> GetCatalogTypesFromFile()
        {
            ...
            return File.ReadAllLines(csvFileCatalogTypes).Skip(1) // skip header row
            .Select(x => CreateCatalogType(x)).Where(x => x != null);
        }
        ...
        static IEnumerable<CatalogBrand> GetCatalogBrandsFromFile()
        {
            ...
            return File.ReadAllLines(csvFileCatalogBrands).Skip(1) // skip header row
            .Select(x => CreateCatalogBrand(x)).Where(x => x != null);
        }
        ...
        static IEnumerable<CatalogItem> GetCatalogItemsFromFile(CatalogDBContext context)
        {
            ...
            return File.ReadAllLines(csvFileCatalogItems).Skip(1) // skip header row
            .Select(row => Regex.Split(row, ",(?=(?:[^\"]*\"[^\"]*\")*[^\"]*$)")).Select(column => CreateCatalogItem(column, csvheaders, catalogTypeIdLookup, catalogBrandIdLookup)).Where(x => x != null);
        }
